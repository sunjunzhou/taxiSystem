...
<script>
  ...
  table.render({
    elem: '#demo'
    ,url: '${path}/punishCheck/punishChecklist' //数据接口
    ,page: true //开启分页
    ,limit:8
    ...
    ,cols: [[ //表头
      ...
      ,{field: 'driver', title: '司机姓名', width:'8%',align:'center'}
      ,{field: 'score', title: '分数', width:'6%',align:'center',templet:function(data){
    	  return "-"+data.score;
      }}
      ,{field: 'why', title: '违章原因', width:'15%',align:'center'}
      ,{field: 'address', title: '违章地点', width:'15%',align:'center'}
      ,{field: 'shortName', title: '所属公司', width:'8%',align:'center'}
      ,{field: 'who_check', title: '提审人', width:'10%',align:'center',templet:function(data){
    	  if(data.who_check == null)
    		  return "暂无提审人员";
    	  else
    		  return data.who_check;
      }}
      ,{field: 'successTime', title: '提审时间', width:'15%',align:'center',templet:function(data){
    	  if(data.successTime == null)
    		  return "暂无审查时间";
    	  else
    		  return updateDateTime(data.successTime);
      }}
      ,{field: 'isType', title: '审核状态', width:'9.5%',align:'center',templet:function(data){
    	  if(data.isType == 1){
    		  return "审核已通过";
    	  }else{
    		  return "审核未通过";
	...
	$("#check-btn").click(function(){
		...
		if(data.length==0){
			...
			layer.confirm('是否确认送审所选中的信息?', {icon: 3, title:'提示',offset: '150px'}, function(index){
				$.post("${path}/punishCheck/sendPunish",{"res":res,"res1":res1,"res2":res2},function(data){
					if(data==0){
