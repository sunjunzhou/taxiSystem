...
<form class="layui-form" action="" id="dept-form" lay-filter="dept-form-filter">
    ...
    <div class="layui-inline">
      ...
      <div class="layui-input-inline">
        <input type="text" name="describe" lay-verify="required"  class="layui-input">
      </div>
    </div>
    </div>
  <div class="layui-form-item">
    <div class="layui-input-block">
      <button class="layui-btn" lay-submit="" lay-filter="subdeptform">保存</button>
 ...
 <script type="text/html" id="deptform_1">
<form class="layui-form" action="" id="dept-form_1" lay-filter="dept-form-filter_1">
    <div class="layui-form-item">
    <div class="layui-inline">
    ...
    <div class="layui-inline">
      ...
      <div class="layui-input-inline">
        <input type="text" name="creator" lay-verify="required"  class="layui-input">
      </div>
    </div>
    <div class="layui-inline">
  ...
  <div class="layui-form-item">
    <div class="layui-input-block">
      <button class="layui-btn" lay-submit="" lay-filter="subdeptform_1">保存</button>
      <button type="reset" class="layui-btn layui-btn-primary">重置</button>
    </div>
  </div>
   </form>
...
<script>
	...
	$("#add-btn").click(function() {
		index=layer.open({
			title:['新增部门信息', 'font-size:18px;'],
			type:1,
			resize:false,
			content:$("#deptform_1").html(),
			 offset: '50px'
			});
	});
	
	form.on('submit(subdeptform_1)', function(data){
		$.post("${path}/dept/insertDeptInfo",$("#dept-form_1").serialize(),function(flag){
			  if(flag==0){
	...
	$("#update-btn").click(function() {
		...
		if(data.length==0){
			...
			//修改查询用户信息
			index=layer.open({
			title:['编辑部门信息', 'font-size:18px;'],
			type:1,
			resize:false,
			content:$("#deptform").html(),
			 offset: '50px'
			});
			$.post("${path}/dept/getdeptById",{id:data[0].id},function(data){
				form.val("dept-form-filter",{
					"id":data.id,
					"name":data.name,
	...
	$("#del-btn").click(function(){
			...
			if(checkStatus.data.length==0){
				...
				layer.confirm('是否确认删除所选中的信息?', {icon: 3, title:'提示',offset: '150px'}, function(index){
					$.post("${path}/dept/deleteDeptById",{"res":res},function(data){
						if(data==0){
  ...
  table.render({
    ...
    ,cols: [[ //表头
      ...
      ,{field: 'name', title: '部门名称', width:'15%',align:'center'}
      ,{field: 'describe', title: '部门描述', width:'35%',align:'center'}
      ,{field: 'createtime', title: '创建时间', width:'22.6%',align:'center',templet:function(data){
    	  return updateDate(data.createtime);
      }} 
      ,{field: 'operator_time', title: '操作时间', width:'23%',align:'center',templet:function(data){
    	  return updateDateTime(data.operator_time);
      }}
    ]]
  });
