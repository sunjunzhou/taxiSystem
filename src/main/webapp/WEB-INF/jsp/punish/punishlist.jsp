...
<form class="layui-form" action=""  lay-filter="form-info">
 <div class="layui-form-item">
    ...
    <div class="layui-inline">
      <label class="layui-form-label">违章原因</label>
      <div class="layui-input-inline">
...
<div class="layui-inline">
      ...
      <div class="layui-input-inline">
         <input type="text" name="createTime" disabled="disabled"  class="layui-input">
      </div>
    </div>
    <div class="layui-inline">
      <label class="layui-form-label">稽查人</label>
      <div class="layui-input-inline">
...
<div class="layui-form-item">
     <div class="layui-inline">
      <label class="layui-form-label">选择违章原因</label>
      <div class="layui-input-inline">
...
<script>
  ...
  table.render({
    elem: '#demo'
    ,url: '${path}/punish/getAllPunishList' //数据接口
    ,page: true //开启分页
    ,limit:8
    ...
    ,cols: [[ //表头
      ...
      ,{field: 'driver', title: '司机姓名', width:'10%',align:'center'}
      ,{field: 'why', title: '违章原因', width:'11.7%',align:'center'}
      ,{field: 'who_send', title:'稽查人', width:'10%', align:'center'}
      ,{field: 'address', title:'违章地点', width:'22%', align:'center'}
      ,{field: 'shortName', title: '所属公司', width:'10%',align:'center'}
      ,{field: 'isType', title: '状态', width:'10%', align:'center',templet:function(data){
    	  if(data.isType==1){
			  return "已审核";
		  }else{
			  if(data.isSend == 0)
...
1
	...
	$("#add-btn").click(function() {
		...
		$.post('${path}/punish/getAllCarInfo',function(data){
		...
		$.post('${path}/punish/getAllPunishInfo',function(data){
			  
			  var str="<option value=''>请选择违章原因</option>";
			  for (var i = 0; i < data.length; i++) {
		...
		form.on('submit(formbtn)', function(data){
			  $.post("${path}/punish/editpunish",$("#add-form").serialize(),function(data){
					if(data==0){
...
$("#update-btn").click(function() {
	...
	if(data.length==0){
		...
		if(data[0].isType == 1){
			...
			$.post("${path}/punish/getPunishInfoById",{id:data[0].id},function(data){
				...
				form.val("add-form",{
				 ...
				 $.post("${path}/punish/getPunishInfoById",{id:data[0].id},function(data){
					  var str="<option value=''>请选择违章原因</option>";
					  for (var i = 0; i < data.punishlist.length; i++) {
						str+="<option value='"+data.punishlist[i].id+"'";
						if(data.why == data.punishlist[i].id){
							str += "selected";
						}
						str += ">"+data.punishlist[i].name+"</option>"
					}
					  $("#select").html(str);
					  form.render("select"); 
				 });
...
$("#check-btn").click(function(){
	...
	if(checkStatus.data.length==0){
		...
		layer.confirm('是否确认送审所选中的信息?', {icon: 3, title:'提示',offset: '150px'}, function(index){
			$.post("${path}/punish/sendpunish",{"res":res},function(data){
				if(data==0){
	...
	$("#del-btn").click(function(){
		...
		if(checkStatus.data.length==0){
			...
			layer.confirm('是否确认删除所选中的信息?', {icon: 3, title:'提示',offset: '150px'}, function(index){
				$.post("${path}/punish/deletepunishById",{"res":res},function(data){
					if(data==0){
