...
<script>
  ...
  table.render({
    elem: '#demo'
    ,url: '${path}/driverCheck/driverChecklist' //数据接口
    ,page: true //开启分页
    ,limit:8
    ...
    ,cols: [[ //表头
      ...
      ,{field: 'sex', title: '性别', width:'8%',align:'center'}
      ,{field: 'tel', title: '电话', width:'10.5%',align:'center'}
      ,{field: 'card_Id', title: '身份证号', width:'13%',align:'center'}
      ,{field: 'address', title: '家庭地址', width:'12%',align:'center'}
      ,{field: 'Native', title: '籍贯', width:'8%',align:'center'}
      ,{field: 'carId', title: '车辆', width:'10%',align:'center'}
      ,{field: 'companyName', title: '所属公司', width:'15%',align:'center'}
      ,{field: 'isType', title: '审核状态', width:'10.2%',align:'center',templet:function(data){
    	  if(data.isType == 1){
    		  return "审核已通过";
    	  }else{
    		  return "审核未通过";
	...
	$("#check-btn").click(function(){
		...
		if(checkStatus.data.length==0){
			...
			layer.confirm('是否确认送审所选中的信息?', {icon: 3, title:'提示',offset: '150px'}, function(index){
				$.post("${path}/driverCheck/sendDriver",{"res":res},function(data){
					if(data==0){
